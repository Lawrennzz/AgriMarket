-- Add view tracking columns to product_stats if they don't exist
ALTER TABLE product_stats
ADD COLUMN IF NOT EXISTS total_view_clicks INT DEFAULT 0,
ADD COLUMN IF NOT EXISTS daily_views INT DEFAULT 0,
ADD COLUMN IF NOT EXISTS weekly_views INT DEFAULT 0,
ADD COLUMN IF NOT EXISTS monthly_views INT DEFAULT 0,
ADD COLUMN IF NOT EXISTS last_view_click_date DATETIME DEFAULT NULL;

-- Create view tracking procedure
DELIMITER //

CREATE PROCEDURE IF NOT EXISTS update_product_view_stats(IN p_product_id INT)
BEGIN
    -- Update existing record if exists
    UPDATE product_stats 
    SET 
        total_view_clicks = total_view_clicks + 1,
        daily_views = daily_views + 1,
        weekly_views = weekly_views + 1,
        monthly_views = monthly_views + 1,
        last_view_click_date = NOW()
    WHERE product_id = p_product_id;
    
    -- Insert new record if doesn't exist
    IF ROW_COUNT() = 0 THEN
        INSERT INTO product_stats 
            (product_id, total_view_clicks, daily_views, weekly_views, monthly_views, last_view_click_date)
        VALUES 
            (p_product_id, 1, 1, 1, 1, NOW());
    END IF;
END //

-- Create procedure to reset periodic counters
CREATE PROCEDURE IF NOT EXISTS reset_periodic_views()
BEGIN
    -- Reset daily views
    UPDATE product_stats 
    SET daily_views = 0 
    WHERE DATE(last_view_click_date) < CURDATE();
    
    -- Reset weekly views
    UPDATE product_stats 
    SET weekly_views = 0 
    WHERE YEARWEEK(last_view_click_date) < YEARWEEK(NOW());
    
    -- Reset monthly views
    UPDATE product_stats 
    SET monthly_views = 0 
    WHERE MONTH(last_view_click_date) < MONTH(NOW()) 
       OR YEAR(last_view_click_date) < YEAR(NOW());
END //

-- Create view for most viewed products report
CREATE OR REPLACE VIEW most_viewed_products AS
SELECT 
    p.id AS product_id,
    p.name AS product_name,
    p.price,
    COALESCE(ps.total_view_clicks, 0) as total_views,
    COALESCE(ps.daily_views, 0) as daily_views,
    COALESCE(ps.weekly_views, 0) as weekly_views,
    COALESCE(ps.monthly_views, 0) as monthly_views,
    ps.last_view_click_date
FROM 
    products p
LEFT JOIN 
    product_stats ps ON p.id = ps.product_id
ORDER BY 
    ps.total_view_clicks DESC //

DELIMITER ;

-- Create event to automatically reset periodic views
CREATE EVENT IF NOT EXISTS reset_periodic_views_event
ON SCHEDULE EVERY 1 DAY
STARTS CURRENT_DATE + INTERVAL 1 DAY
DO CALL reset_periodic_views(); 